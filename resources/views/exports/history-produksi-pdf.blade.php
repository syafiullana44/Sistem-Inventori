...
<head>
    ...
    <style>
        * { font-family: Arial, sans-serif; font-size: 10px; }
        ...
        .detail-row td { background: #f8fafc; font-size: 9px; padding-left: 20px; }
...
<table>
    ...
    <tbody>
        @forelse($permintaan as $item)
            ...
            <tr>
                @if($index == 0)
                    <td rowspan="{{ $item->details->count() }}">{{ $item->kode_pr }}</td>
                    ...
                    <td rowspan="{{ $item->details->count() }}">
                        <span class="
                            ...
                            @elseif($item->status == 'Menunggu') badge-menunggu
                ...
                <td class="text-center">{{ $detail->jumlah_dikeluarkan }}</td>
                @if($index == 0)
                    <td rowspan="{{ $item->details->count() }}">{{ $item->tanggal_diproses ? \Carbon\Carbon::parse($item->tanggal_diproses)->format('d/m/Y H:i') : '-' }}</td>
    ...
    <tfoot>
        <tr class="total-row">
            <td colspan="7" class="text-right">Total Permintaan:</td>

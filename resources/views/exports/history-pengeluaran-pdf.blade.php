        ...
        @forelse($pengeluaran as $item)
            ...
            <tr>
                @if($index == 0)
                    ...
                    <td rowspan="{{ $item->details->count() }}">{{ $item->userProduksi->nama_lengkap ?? '-' }}</td>
                    <td rowspan="{{ $item->details->count() }}">{{ \Carbon\Carbon::parse($item->tanggal_selesai)->format('d/m/Y H:i') }}</td>
                @endif
                ...
                @if($index == 0)
                    <td rowspan="{{ $item->details->count() }}"><span class="badge-selesai">Selesai</span></td>
                @endif
            </tr>
            @endforeach
        @empty

...
<div class="card-custom">
    <form method="POST" action="{{ route('produksi.permintaan.store') }}" id="formPermintaan">
        @csrf

        <div class="mb-3">
            <label class="form-label">Catatan / Keterangan</label>
            <textarea name="keterangan" class="form-control" rows="2" placeholder="Contoh: Untuk produksi meja ukir 5 unit"></textarea>
        </div>
                    ...
                    <div class="col-md-6">
                        <select name="bahan[0][id]" class="form-select" required>
                            ...
                            <option value="{{ $item->id }}" data-satuan="{{ $item->satuan }}">
                                {{ $item->kode_bahan }} - {{ $item->nama_bahan }} {{ $item->deskripsi ? '- ' . $item->deskripsi : '' }} (Stok: {{ $item->stok_saat_ini }})
                            </option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('produksi.dashboard') }}" class="btn btn-secondary">Batal</a>
